...
function updateSelectedContact(user_id, model) {
 console.log(model);

 $(document).find(".messenger-list-item").removeClass("m-list-active");
 $(document)
     .find(
         `.messenger-list-item[data-contact=` + (user_id || getMessengerId()) + `][data-model="${model}"]`
     )
 ...
 if (messenger != 0) {
     ...
     $.ajax({
         ...
         success: (data) => {
             ...
             $(".messenger-infoView")
                 .find(".avatar")
                 .css("background-image", 'url("' + data.fetch.image + '")');
             $(".header-avatar").css(
                 "background-image",
                 'url("' + data.fetch.image + '")'
             );
...
channel.bind("messaging", function (data) {
 ...
 if (currentPath === '/support-chat') {
     let messageTime = $(data.message).find('.message-time .time').text().trim();
     let messageText = $(data.message).find('.message').contents().filter(function () {
         return this.nodeType === 3; // Node.TEXT_NODE
     }).text().trim();
     $(".messenger-list-item[data-contact=" + data.from_id + "] tr span:not(.contact-item-time)").html(`<span style="font-weight: bold;">${messageText}</span>`);
     $(".messenger-list-item[data-contact=" + data.from_id + "] tr p span.contact-item-time").text(`${messageTime}`);

     //increase counter
     let unseenCounterElement = $(".messenger-list-item[data-contact=" + data.from_id + "] tr div.unseen-counter b");

     if (unseenCounterElement.length > 0) {
         let currentCounter = parseInt(unseenCounterElement.text()) || 0;
         unseenCounterElement.html(`<b>${currentCounter + 1}</b>`);
     } else {
         $(".messenger-list-item[data-contact=" + data.from_id + "] tr div.unseen-counter").html(`<b>1</b>`)
     }
 } else if (currentPath.startsWith('/chatify/')) {
     ...
     $(".messenger-list-item[data-contact=" + getMessengerId() + "]")
         .find("tr>td>div.unseen-counter>b")
         .remove();
 }
 ...
 if (user_id != auth_id) {
     $.ajax({
         ...
         success: (data) => {
             ...
             if (user_id == getMessengerId()) updateSelectedContact(user_id, $(this).attr('data-model'));
...
$(document).ready(function () {
 ...
 $("body").on("click", ".messenger-list-item", function () {
     ...
     updateSelectedContact(userID, $(this).attr('data-model'));
 });
 ...
 $("body").on("click", ".favorite-list-item", function () {
     ...
     setMessengerId(uid);
     IDinfo(uid);
     updateSelectedContact(uid, $(this).attr('data-model'));
